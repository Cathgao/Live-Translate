...
实时语音翻译的桌面客户端。**无边框 + 不透明窗口**，使用 WebView2 作为渲染层，连接 `.env` 中 `VITE_WS_URL` 指定的 WebSocket 端点，并内置 **UART 串口**把译文流式推送给外部 MCU / 串口屏。

整个 `desktop-client/` 目录是**自包含**的：放到任何装了前置依赖的 Windows 机器上，先复制 `.env.example` 为 `.env` 并填入服务端地址，再 `npm install && npm run tauri:dev` 就能跑。
...
- **UART 串口**：
  ...
  - 折叠面板显示最近的 RX 字节。
- 所有设置（语言、VAD、字号、串口配置、Token 用量）持久化到 `localStorage`。
...
│   ├── ws.ts                        # LiveClient（WS 消息分发）
│   ├── serial.ts                    # Tauri 命令包装 + 事件监听
│   ├── settings.ts                  # 设置 + Token 用量持久化
...
└── src-tauri/                       # Rust 后端
    ...
    └── src/
        ├── main.rs
        ├── lib.rs                   # 4 个 Tauri 命令：serial_list / open / close / write
...
```bash
cd desktop-client
cp .env.example .env       # 然后编辑 .env，把 VITE_WS_URL 改成你的服务端地址
npm install
npm run tauri:dev
```
...
- 之后的增量编译是秒级。
- 应用启动后会开一个最大化无边框窗口，自动连接 `.env` 里 `VITE_WS_URL` 指向的服务端。
...
连接地址由 `.env` 里的 `VITE_WS_URL` 决定，启动时拼上查询参数：

```
<VITE_WS_URL>?source=Auto&target=Chinese%20(Simplified)&silenceMs=1000[&token=...]
```
